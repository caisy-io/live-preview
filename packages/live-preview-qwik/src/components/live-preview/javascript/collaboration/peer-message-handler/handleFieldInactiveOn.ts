...
export const handleFieldInactiveOn = (state: ICollaborationState, peer: IPeer, data: Uint8Array) => {
...
const removeActiveFieldCollaborator = ({
    ...
    if (state.fieldCollaborator[fieldKey]) {
        state.fieldCollaborator[fieldKey] = state.fieldCollaborator[fieldKey].filter((u: any) => u !== clientId);
