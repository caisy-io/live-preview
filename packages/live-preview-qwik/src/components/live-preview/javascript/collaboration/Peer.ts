import {SimplePeer} from "../../simple-peer/index.js";
...
import { encodeAnyJSONMessage, encodeFieldMessage, uint8ArrayToBase64Unicode } from "./encoding";
...
export class Peer extends Observable<any> {
    ...
    init() {
        ...
        state.lastOwnActiveFieldMessage && this.send(state.lastOwnActiveFieldMessage);
    }
    destroy() {
        ...
        if (this.connectedLocal && this.lastPing > Date.now() - 1500) {
            state.localBroadcastChannel?.postMessage(
                JSON.stringify({
                    ...
                    from: state.ownClientId,
                    to: this.clientId,
                    data: uint8ArrayToBase64Unicode(data),
                }),
            );
            return;
        }
    ...
    setupP2P() {
        ...
        this.p2p.on("data", (data: any) => {
            onPeerMessage(state, this, data);
        });

        this.p2p.on("signal", (data: any) => {
