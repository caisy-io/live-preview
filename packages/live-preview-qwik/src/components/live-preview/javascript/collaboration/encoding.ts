...
export const encodeFieldMessage = ({
    ...
    const encodedMeta: any = new TextEncoder().encode(
        `:${documentId}:${documentFieldLocaleId}:${blueprintFieldId}:${syncChecksum || 0}:`,
    );
    return new Uint8Array([messageType, ...encodedMeta, ...(body ? body : [])]);
};

export const encodeAnyJSONMessage = ({ messageType, body }: { messageType: number; body?: any }): Uint8Array => {
...
export const encodeHotFieldUpdate = ({
    ...
    if (metaKey.length != 144) throw new Error(`Meta key wrong length: ${metaKey.length} != 144`);
    ...
    return new Uint8Array([PEER_MESSAGE_TYPE_HOT_FIELD_UPDATE, ...encodedMeta, ...encodeData]);
};
...
export function uint8ArrayToBase64Unicode(bytes: Uint8Array): string {
    let binary = "";
...
export function base64ToUint8ArrayUnicode(base64: string): Uint8Array {
...
export function uint8ArrayToBase64Binary(bytes: Uint8Array): string {
    let binary = "";
    const len = bytes.byteLength;
    for (let i = 0; i < len; i++) {
        binary += String.fromCharCode(bytes[i]);
...
export function base64ToUint8ArrayBinary(base64: string): Uint8Array {
