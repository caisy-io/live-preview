...
import { broadcastToAllPeers, clientIsPreviewRole } from "../collaboration/helper";
import { ATTRIBUTE_NAME_LOCALE_NAME, ATTRIBUTE_NAME_DOCUMENT_ID, ATTRIBUTE_NAME_FIELD_NAME } from "../constants";
...
export const startInspectMode = ({ locale, projectId }: { locale: string; projectId: string }) => {
    ...
    const updateElementPosition = () => {
        ...
        if (currentRectOfElement && currentRectOfParentOfElement) {
            let upperBoundOfTooltip = currentRectOfElement.top - INTERACTIVE_ELEMENT_HEIGHT;
    ...
    const addInteractiveElementOnHover = (e: any) => {
        ...
        for (const eventTarget of eventTargets) {
            ...
            const localeApiName = element.getAttribute(ATTRIBUTE_NAME_LOCALE_NAME) ?? defaultLocale;

            if (fieldName && documentId && localeApiName) {
                positioningElement = element;

                if (updateElementPosition()) {
                    interactiveElement.setAttribute(ACTIVE_FIELD_NAME_ATTRIBUTE, fieldName);
                    interactiveElement.setAttribute(ACTIVE_DOCUMENT_ID_ATTRIBUTE, documentId);
                    interactiveElement.setAttribute(ACTIVE_LOCALE_NAME_ATTRIBUTE, localeApiName);
                }
                break;
            }
        }
    ...
    const clickHandler = () => {
        const activeDocumentId = interactiveElement.getAttribute(ACTIVE_DOCUMENT_ID_ATTRIBUTE);
        const activeFieldName = interactiveElement.getAttribute(ACTIVE_FIELD_NAME_ATTRIBUTE);
        const activeLocale = interactiveElement.getAttribute(ACTIVE_LOCALE_NAME_ATTRIBUTE) || defaultLocale;
        ...
        if (activeFieldName && activeDocumentId && activeLocale) {
            ...
            const sameUserEditorPeers = window.c.collaboration?.peers.filter(
                (p: any) =>
                    ...
                    p.clientId.slice(0, 38) == window.c.collaboration?.ownClientId?.slice(0, 38), // only same user id get the updates
            );
