...
import * as Stream from "@caisy/readable-stream";
...
class Peer extends Duplex {
    constructor(opts) {
        ...
        this._id = getRandomBytes(4).toString("hex").slice(0, 7);
        this._debug("new peer %o", opts);

        this.channelName = opts.initiator ? opts.channelName || getRandomBytes(20).toString("hex") : null;
    ...
    replaceTrack(oldTrack, newTrack, stream) {
        ...
        if (sender.replaceTrack != null) {
            ...
            this.destroy(
                errCode(new Error("replaceTrack is not supported in this browser"), "ERR_UNSUPPORTED_REPLACETRACK"),
            );
        }
    }
    ...
    _onChannelMessage(event) {
        if (this.destroyed) return;
        let data = event.data;
        if (data instanceof ArrayBuffer) data = data; // this was modified - before here was a Buffer.from(data) 
        this.push(data);
    }
